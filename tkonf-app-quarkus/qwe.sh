...
curl  -v -X GET http://localhost:8080/api/crypto/health

# 2. Получение публичного ключа (должно работать)
curl  -v -X GET http://localhost:8080/api/crypto/public-key

# 3. Подпись сообщения - ВАЖНО: используйте точный формат JSON
curl  -v -X POST http://localhost:8080/api/crypto/sign \
